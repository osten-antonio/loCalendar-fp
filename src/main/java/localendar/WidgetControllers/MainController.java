...
public class MainController implements Initializable {
    ...
    private Map<String, Map<LocalDate, ArrayList<Node>>> cache;
    ...
    private void fillMonth() {
        ...
        if (cache.containsKey(curDate.toString())) {
            ...
            if (dayToNodes != null) {
                ...
                Map<LocalDate, ArrayList<Node>> dayToNodes = new HashMap<>();
        ...
        public void refreshTaskList (ArrayList < Task > tasks) {
         ...
         */
            ...
            // Loop through all tasks and check if the title contains the search text
            for (Task task : tasks) {


                if (task.getTitle().contains(searchBar.getText())) {
                    temp.add(task);  // If the task title contains the search text, add it to the filtered list
                }
            }
         ...
         */
            ...
            for (Task task : tasks) {
                ...
                if (completedFilter && !task.isStatus()) matches = false;
                if (uncompletedFilter && task.isStatus()) matches = false;

                if (enableDateRange.isSelected()) {
                    if (fromDateFilter != null && task.getDueDate().isBefore(fromDateFilter)) matches = false;
                    if (toDateFilter != null && task.getDueDate().isAfter(toDateFilter)) matches = false;
                }
                ...
                if (!rRuleFilter.isEmpty() && task.getRrule() != null) {
                    boolean matchesRRule = rRuleFilter.stream().anyMatch(rule -> task.getRrule().toString().contains(rule));
                    if (!matchesRRule) matches = false;
                }

                if (matches) filteredList.add(task);
            }
            ...
            if (selectedToggle != null) {
                ...
                if (selectedRadio == priorityRadio) {
                    comparator = Comparator.comparing(Task::getPriority, Comparator.comparingInt(Enum::ordinal));
